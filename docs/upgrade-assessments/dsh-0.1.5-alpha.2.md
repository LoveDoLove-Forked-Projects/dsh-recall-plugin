...
> 评估对象：[dsh-v0.1.5-alpha.2](https://github.com/deepseek-ai/deepseek-harness/releases/tag/dsh-v0.1.5-alpha.2)（commit `b2e3b2a`，2026-09-09 发布；pre-release，**npm dist-tag `alpha` 已指向本版**，`npm install -g @deepseek-ai/dsh@0.1.5-alpha.2` 全局实装）
> 本地基线：dsh 0.1.5-alpha.2（本次全局实装）；对照源：[dsh-0.1.5-alpha.1.md](./dsh-0.1.5-alpha.1.md)
> 评估方式：release notes 筛查 + **官方 dsh-v0.1.5-alpha.2 tag 源码实证**（ui-chat 槽位契约 / session-controller fork 契约 / slots.md 层级树 / 面板与文件交付决策文档）+ **三层门禁实跑**（check:dsh 报漂移/peer 越界 + test:probe 31 项探针全绿 + verify:host 装配断言全过）+ **全局实装包 tree-SHA diff**（插件消费类型源 alpha.1↔alpha.2 零变动）
> 总结论：**接口层面零破坏、行为层面无回归——无需任何代码修改。** 核心三链（撤回主链路 fork、快照触发/冷读、撤回按钮 UI 渲染）依赖的契约在 alpha.2 全部逐字保留；唯一结构变化（conversation 槽位迁入 `main.conversation`）是宿主面板挂载层调整，按 key 注入的插件注册不受影响。需跟进项（peer 范围越界 / 镜像漂移）已随本次同步处理完毕，非代码缺陷。
...
- **npm 发布状态**：`npm view @deepseek-ai/dsh dist-tags` 实证 `alpha` 已指向 `0.1.5-alpha.2`，本次已 `npm install -g @deepseek-ai/dsh@0.1.5-alpha.2` 全局实装并跑完三层门禁。
...
1. ~~等待官方推包~~——已完成：`alpha` 已指向 0.1.5-alpha.2，本次 `npm install -g @deepseek-ai/dsh@0.1.5-alpha.2` 全局实装。
2. ~~三层门禁复跑~~——已完成：`test:probe` 31/31、`verify:host` 装配断言全过；`check:dsh` 报漂移/peer 越界，转入第 3/4 步处理。
3. ~~同步兼容声明~~——已完成：package.json `dshReleases` 矩阵补 `0.1.5-alpha.2`、7 个 dsh-* peer 范围补 `|| >=0.1.5-alpha.2 <=0.1.5-alpha.2` tuple（沿 9c3c56b 先例）。
4. ~~同步契约文档~~——已完成：reference/ 镜像重拉（仅 09-architecture.md 一行措辞变化，其余 12 源 SHA 未动）、reference/README 归档版本字段与 dsh-contract.md「对应版本」同步至 0.1.5-alpha.2、compat-audit 追加核验段。
...
| npm 已推包 + 全局实装 | `npm view @deepseek-ai/dsh dist-tags`：`alpha` 指向 0.1.5-alpha.2；`dsh --version` 实跑 0.1.5-alpha.2 |
| 三层门禁实跑 | `npm run test:probe`（31/31）、`npm run verify:host`（inject=shell,sessions,webServer,agents，端点 12 项）通过；`npm run check:dsh` 同步 peer/镜像后全绿 |
| 依赖面源码零 diff | 全局实装包 tree-SHA 比对 alpha.1↔alpha.2：`dsh-session/lib/types/types.d.ts`、ui-chat/ui-conversation `slots.d.ts`、session-controller `sessions.d.ts`、settings-plugins `slot-contract.d.ts` 均未变 |
