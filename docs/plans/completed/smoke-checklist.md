...
## 八、仅撤回对话模式批次（Windows 侧必测，发版前置；[plan-session-only.md](./plan-session-only.md)）

> 2026-09-18 实施：单测 366 例全绿（新增 routes-scope 5 例）+ verify:host 通过，本节是真实 DSH 链路的活体验收。

- [x] **S-1 session-only 全链**：改文件 → 撤回选「仅撤回对话」→ 文件保持改动后状态（字节级对照）、对话回退、原会话归档、输入框回填被撤回消息、版本家族记录正常 ✅ 手动改动后 MD5 前后一致（E6B14FB8）；零新 `snap-pre-rollback-*` tag；子会话打开仅含消息 1、标题继承；m2 文本回填输入框；原会话从侧栏消失；lineage 6→7 条
- [x] **S-2 默认 both 回归**：默认「回退文件与对话」与现状逐项一致 ✅ 重开面板默认复位 both（上次 session-only 选择不残留）；确认回退后文件回退到快照状态（手动改动被覆盖）、安全快照照打（pre-rollback +1）、对话回退 + lineage +1（7→8）
